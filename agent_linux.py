#!/usr/bin/env python3
"""
VPrint Printer Agent for Linux/Raspberry Pi
Polls Supabase for print jobs and sends them to CUPS (Common Unix Printing System)
Compatible with Raspberry Pi Zero W, Ubuntu, Debian, CentOS
"""
...
# Ensure stdout uses UTF-8 to prevent crashes with non-ASCII characters
...
# -------------------------
# Printer Detection & Health Check
# -------------------------
def get_available_printers():
    """List all available printers on the system using lpstat."""
    try:
        result = subprocess.run(['lpstat', '-p', '-d'], capture_output=True, text=True, timeout=10)
        logger.info(f"Available printers:\n{result.stdout}")
...
def check_printer_status(printer_name=None):
    ...
    try:
        if printer_name:
            result = subprocess.run(['lpstat', '-p', printer_name], capture_output=True, text=True, timeout=10)
            ...
            logger.info("No printer name specified. Will use default printer.")
            return True
    except Exception as e:
...
# -------------------------
# Printing Logic for Linux/CUPS
# -------------------------
def print_file(file_path, copies=1, start_page=None, end_page=None):
    """
    Prints a file using lp command (CUPS) on Linux.
    Supports page range and copies on Raspberry Pi, Ubuntu, Debian, etc.
    """
    ...
    if start_page or end_page:
        if start_page and end_page:
            ...
            cmd.extend(['-P', f'{start_page}-'])
        elif end_page:
            cmd.extend(['-P', f'-{end_page}'])
    ...
    try:
        ...
        logger.info(f"Print job sent successfully. CUPS response: {result.stdout}")
    except subprocess.CalledProcessError as e:
        raise Exception(f"lp command failed with exit code {e.returncode}: {e.stderr}")
    except FileNotFoundError:
        raise Exception("lp command not found. CUPS (Common Unix Printing System) not installed. Run: sudo apt-get install cups")
    except Exception as e:
...
def main():
    logger.info("=" * 60)
    logger.info(f"VPrint Agent Started (Linux) | Printer: {PRINTER_ID}")
    logger.info("=" * 60)
